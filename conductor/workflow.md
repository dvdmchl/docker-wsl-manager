...
9. **Attach Task Summary with Git Notes:**
   - **Step 9.1: Get Commit Hash:** After the user confirms the commit, obtain the hash of the *just-completed commit* (`git log -1 --format="%H"`).
   - **Step 9.2: Draft Note Content:** Create a detailed summary for the completed task. This should include the task name, a summary of changes, a list of all created/modified files, and the core "why" for the change.
   - **Step 9.3: Attach Note:** Use the `git notes` command to attach the summary to the commit (or ask the user to run it if write access is restricted).
     ```bash
...
7.  **Attach Auditable Verification Report using Git Notes:**
    ...
    -   **Step 7.2: Attach Note:** Use the `git notes` command (or ask user) and the full commit hash from the previous step to attach the full report to the checkpoint commit.
...
```bash
# Verify (includes SpotBugs and Checkstyle)
mvn clean verify

# SonarQube (CRITICAL: Must pass Quality Gate)
